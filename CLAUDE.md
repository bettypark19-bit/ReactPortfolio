...
## 6. 배포 규칙 (CRITICAL)
- **배포(push/build) 전 반드시 `PREDEPLOY.md` 체크리스트를 확인할 것**
- 체크리스트 미통과 항목이 있으면 배포 금지
- 새 이미지 추가 시: WebP 변환 → 경로 업데이트 → lazy 적용 순서 준수
- 새 라이브러리 추가 시: `manualChunks` vendor 청크 등록 필수
- API 키/시크릿은 반드시 `.env`에만 작성, 소스 코드 하드코딩 금지
